...
class _TeamNamePageState extends State<TeamNamePage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success') {
          setState(() {
            teamNames = List<String>.from(
                data['teams'].map((team) => team['team_name']));
          });
        } else {
          setState(() {
            teamNames = [];
          ...
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text('No teams available')));
        }
      } else {
        setState(() {
          teamNames = [];
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to load teams. Server error.')));
      }
    } catch (error) {
      setState(() {
        teamNames = [];
  ...
  Future<void> addTeam() async {
    if (newTeamName.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Please enter a team name')));
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success') {
          ...
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text('Team added successfully')));
        } else {
          ScaffoldMessenger.of(context)
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error occurred while adding the team')));
      }
    } catch (error) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Error occurred: $error')));
                ...
                DropdownButton<String>(
                  ...
                  items: teamNames.map((teamName) {
                    return DropdownMenuItem<String>(
                        value: teamName, child: Text(teamName));
                  }).toList(),
                ),
                          ...
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) => PointCounter(
                                ...
                                refereeName: widget
                                    .refereeName, // Pass referee name here
